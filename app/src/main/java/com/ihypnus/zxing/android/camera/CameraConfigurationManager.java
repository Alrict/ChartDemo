...
public final class CameraConfigurationManager {
    ...
    public void initFromCameraParameters(Camera camera) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            manager.getDefaultDisplay().getRealSize(displayPoint);
        } else {
            manager.getDefaultDisplay().getSize(displayPoint);
        }
        mCameraResolution = findCloselySize(displayPoint.x, displayPoint.y,
                parameters.getSupportedPreviewSizes());

        setRotation(manager.getDefaultDisplay().getRotation());


        Log.i("camera", displayPoint.x / 2 + "");
        Log.i("camera", displayPoint.y / 2 + "");
        Log.i("camera", mCameraResolution.width + "");
        Log.i("camera", mCameraResolution.height + "");

        mPictureResolution = findCloselySize(displayPoint.x, displayPoint.y,
...
//        Camera.Size afterSize = afterParameters.getPreviewSize();
        ...
        setAutoFocusInternal(camera, true);

        /** 设置相机预览为竖屏 */
        setCameraDisplayOrientation(camera);
    }

    public void setAutoFocusInternal(Camera camera, boolean autoFocus) {
        if (camera != null) {
            Camera.Parameters parameters = camera.getParameters();
    ...
    public void setCameraDisplayOrientation(Camera camera) {
        if (camera != null) {
            int cameraRotation = calcCameraRotation(DISPLAY_ORIENTATIONS.get(mRotation));
            Camera.Parameters parameters = camera.getParameters();
            ...
            Log.i("qrcode", "旋转耗时：" + (System.currentTimeMillis() - start));
        }
    }
