            ...
            for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++) {
                ...
                if (!dataSet.isStacked()) {
                    ...
                    while (index < dataSet.getEntryCount() * mAnimator.getPhaseX()) {
                        Log.d("llw111", "index:" + index + "dataSet count:" + dataSet.getEntryCount() + ",animatorFra:" + dataSet.getEntryCount() * mAnimator.getPhaseX());
                        ...
                        v1 = v1 + v0;
                        float[] newVals = {v0, v1};
//                        entry.setVals(newVals);
                        vals = newVals;
                        Log.d("chartValue", String.valueOf(vals[0] + "," + vals[1]));
                        ...
                        if (vals == null) {
                            ...
                            Log.d("llw", "transformed长度:" + transformed.length);
                            for (int k = 0; k < transformed.length; k += 2) {

                                final float val = vals[k / 2];
                                Log.d("llw111", "val:" + val);
